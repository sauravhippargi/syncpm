...
- **transcripts** — `id`, `user_id` (FK, owner), `title`, `uploaded_at`, `raw_text`, `source` (`manual` | `fathom`), `fathom_meeting_id` (nullable, unique when present — prevents double-importing the same meeting if a webhook fires more than once)
...
- **Assignee resolution** — fetch the connected project's assignable users via Jira's REST API, then do a simple best-effort name match against the extracted owner string (e.g. case-insensitive substring match on first/last name) to pre-select a likely match in the modal's dropdown. This is a convenience default only — the user always confirms or changes it before creating the ticket, so the match doesn't need to be perfect. The dropdown now also surfaces `owner_evidence` alongside the pre-selected name (see below), giving the reviewer something concrete to check the guess against rather than trusting a bare name match.
- **Owner Evidence (`owner_evidence`)** — `ExtractedActionItem` (`lib/prompts/extraction.ts`) gets a new field, `ownerEvidence: string | null`, alongside `owner`. The Gemini `responseSchema` gets a matching nullable-string property, with a field description instructing the model to output the exact transcript quote it used to determine the owner — the phrase that names or directly addresses the person, not a paraphrase. `validateExtractionResult` enforces: if `owner` is non-null but `ownerEvidence` is null/empty, `owner` is set to null — the system never accepts a bare owner assertion with no supporting quote. Since `ActionItemFields.tsx` is the single shared component for owner/due date/status/blockers across Review & Edit, Action Items, and Deadlines, the evidence caption is added there once and appears everywhere `owner` does, with no per-screen duplication. The eval harness (`evals/`) fixture schema and scoring are extended to also check `ownerEvidence` is present and non-empty whenever `owner` is expected to be non-null.
