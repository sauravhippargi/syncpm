...
- **Problem:** Owner assignment (6.2) is a bare model assertion with no supporting evidence, and a wrong owner isn't a display-only issue — it flows into the Jira ticket modal's assignee dropdown as a pre-filled default (6.4), where a reviewer who clicks past it creates a real ticket assigned to the wrong person. Owner is the one extracted field with a direct path to an external side effect, so it's the one worth constraining rather than trusting.
...
- **Where it's shown:** nowhere. This is deliberately a non-visual feature. The evidence quote is extracted, validated against, and stored, but never displayed in the UI — a caption under the owner field and a collapsed icon disclosure were both built and both rejected as visual clutter in a scannable list. What the feature actually buys is the validation guard below, which operates whether or not anyone reads a quote.
- **What it enforces:** extraction cannot assert an owner it can't justify. If the model names an owner but can't cite a supporting quote, the owner is set to null rather than guessed — so an unjustified name never reaches the Jira assignee dropdown's pre-fill (6.4) in the first place. The reviewer sees an empty owner field to fill in, which is a safer default than a confident wrong name they might click past.
- **Explicitly not:** a new approval gate, a blocking validation step, or any change to `is_approved`. A nulled owner is editable exactly like any other empty field.
- **Known cost:** an item whose owner was nulled this way looks identical to one where the model simply found no owner — there's no in-app explanation of why the field is blank. Accepted deliberately: the alternative was surfacing an explanation in the UI, which is the thing being removed.
