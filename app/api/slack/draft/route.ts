...
export async function POST(request: NextRequest) {
  ...
  const item = await prisma.actionItem.findFirst({
    ...
    include: {
      // Only need to know whether a real ticket already exists, not the
      // full sync history — one row is enough (prd.md 6.5).
  ...
  try {
    ...
    // Appended deterministically in code, using the real URL from
    // jira_sync_log — never generated by Gemini, since a hallucinated or
    // slightly-wrong ticket link is worse than no link at all (prd.md 6.5).
    const existingSync = item.jiraSyncLogs[0];
    const fullMessage = existingSync
      ? `${message}\n\nYou can follow it here: ${existingSync.jiraUrl}`
      : message;
