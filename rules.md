...
- **Extraction failure on an async import path** (Fathom webhook, Fathom backfill) — there's no user watching, so "show an in-app error" doesn't apply and the failure must be recorded rather than logged and forgotten. A swallowed failure is indistinguishable from a transcript that genuinely contained no action items, which makes the two impossible to tell apart after the fact. Requirements: the transcript must carry a persisted extraction status (e.g. succeeded/failed) rather than only appearing as a row with zero items; the backfill response must report per-meeting outcomes (imported vs. failed counts, not a single `imported` total); and Transcript History must visibly distinguish "extraction failed" from "0 action items found" so a silently empty import can't pass for a valid one.
- **Empty or unusable transcript text on import** — check for empty/whitespace-only `rawText` *before* creating the transcript row or spending a Gemini call. This applies to the Fathom webhook and backfill paths as well as manual upload; an empty transcript should be skipped with a reason, not imported as a row that can only ever show zero items.
