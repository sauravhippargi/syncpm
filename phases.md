...
- `evals/cases/` — five sample transcripts, each paired with a hand-authored expected output, formalizing what were previously just prose "answer keys" into structured, machine-checkable JSON. Four originals (weekly-cross-functional-sync.txt, engineering-sprint-sync.vtt, customer-escalation-call.srt, casual-team-catchup-vague.txt), plus `discussion-heavy-planning.txt` — *(added later)* a recall-under-soft-assignment case with one dominant speaker and almost no vocative hand-offs, where real tasks surface through deliberation ("that needs to get nailed down," "somebody needs to sit down with legal") and most expected items legitimately have `owner: null`. It exists because the original four are all explicit-assignment meetings while casual-team-catchup-vague rewards restraint, so a prompt tuned to recognize only named hand-offs scores 100% on the suite while badly under-extracting on real discussion-heavy meetings. That exact regression occurred — prompt additions moved a real transcript from 2 items to 0 at temperature 0 — and the suite could not detect it. **Design principle going forward: the suite must measure recall where assignment is implicit, not only where it's explicit.**
