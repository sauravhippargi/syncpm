...
interface ExpectedItem {
  ...
  blockerNoteContains?: string[]; // keyword match, only when a blocker is expected
  note?: string;
}
...
function hasAllKeywords(haystack: string | null, keywords: string[]): boolean {
  if (!haystack) return false;
  const h = haystack.toLowerCase();
  return keywords.every((k) => h.includes(k.toLowerCase()));
}

function hasDueDate(item: ExtractedActionItem): boolean {
  return item.dueDate != null && item.dueDate.trim() !== "";
}

function hasBlocker(item: ExtractedActionItem): boolean {
  return item.blockerNote != null && item.blockerNote.trim() !== "";
}

// The extracted item whose description contains every keyword — how we decide
// "was this expected item found at all."
function findMatch(
...
interface CheckResult {
  label: string; // e.g. "owner", "dueDate", "blockerNote"
  ok: boolean;
}
...
interface ExpectedResult {
  ...
  found: boolean;
  checks: CheckResult[];
  pass: boolean; // found AND every check ok
  failReasons: string[];
}

interface TrialResult {
  expected: ExpectedResult[];
  forbiddenLeaks: string[]; // ids/descriptions of forbidden items that appeared
  optionalViolations: string[]; // optional items that broke a stated constraint
  pass: boolean; // all expected pass, no forbidden leaks, no optional violations
}
...
function scoreExpected(
  ...
  if (item.blockerNoteContains !== undefined) {
    const ok = found && hasAllKeywords(match!.blockerNote, item.blockerNoteContains);
    checks.push({ label: "blockerNote", ok });
    if (found && !ok) {
      failReasons.push(
        `blockerNote: expected keywords [${item.blockerNoteContains.join(", ")}], got "${match!.blockerNote ?? "(none)"}"`
      );
    }
  }
...
// Optional items are never penalized for being present OR absent — but IF the
// model does extract one, any stated constraint must hold (e.g. the Meridian
// item must not be confidently assigned to a named person; a "no rush" item
// must not get a hallucinated hard date). A broken constraint is a real
// precision failure, so it counts against the trial.
function scoreOptional(item: ExpectedItem, extracted: ExtractedActionItem[]): string | null {
  const match = findMatch(extracted, item.descriptionContains);
  if (!match) return null; // absent is fine
...
function runTrial(fixture: Fixture, extracted: ExtractedActionItem[]): TrialResult {
